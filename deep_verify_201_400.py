...
def classify_and_test(issue, programmer_notes):
    ...
    if any(kw in title_lower for kw in ["employee list", "user list", "employee page",
                                         ...
                                         "invite employee"]):
        return test_list_endpoint("/users", "org_admin", description=title)

    # -- Document bugs --
    if "document" in title_lower:
        if "upload" in title_lower:
            return test_api_endpoint("GET", "/documents", "org_admin",
                                     expect_status=200, description=title)
        return test_list_endpoint("/documents", "org_admin",
                                  description=title)
    ...
    if "announcement" in title_lower:
        return test_list_endpoint("/announcements", "org_admin",
                                  description=title)

    # -- Dashboard bugs --
    if "dashboard" in title_lower:
        return test_api_endpoint("GET", "/dashboard/widgets", "org_admin",
                                 expect_status=200, description=title)

    # -- Org chart bugs --
    if "org chart" in title_lower or "org-chart" in title_lower or "organization chart" in title_lower:
        return test_api_endpoint("GET", "/users/org-chart", "org_admin",
                                 expect_status=200, description=title)
    ...
    if "payroll" in title_lower or "salary" in title_lower or "payslip" in title_lower:
        ...
        if "salary" in title_lower:
            return test_api_endpoint("GET", "/salary-structures", "org_admin", base_url=base,
                                     description=title)
    ...
    if "helpdesk" in title_lower:
        return test_api_endpoint("GET", "/helpdesk/tickets", "org_admin",
                                 description=title)

    # -- Audit log bugs --
    if "audit" in title_lower:
        return test_list_endpoint("/admin/audit-logs", "org_admin", description=title)

    # -- Asset bugs --
    if "asset" in title_lower:
        return test_list_endpoint("/assets", "org_admin", description=title)

    # -- Onboarding bugs --
    if "onboarding" in title_lower or "onboard" in title_lower:
        return test_list_endpoint("/onboarding", "org_admin", description=title)

    # -- Policy bugs --
    if "policy" in title_lower or "policies" in title_lower:
        return test_list_endpoint("/policies", "org_admin", description=title)

    # -- Feedback bugs --
    if "feedback" in title_lower:
        return test_list_endpoint("/feedback", "org_admin", description=title)
...
def main():
    ...
    for idx, issue in enumerate(issues):
        ...
        # Skip already-verified issues (from previous run)
        ...
        if "verified-fixed" in existing_labels or "verified-bug" in existing_labels:
            print(f"  SKIPPED (already has verified label)")
            results["SKIPPED"] += 1
            issue_results.append({"number": num, "title": title, "verdict": "SKIPPED",
